...
variable "s3_bucket_tag_key" {
  type        = string
  description = "S3 bucket tag key for requiring KMS encryption. To activate, make sure that the AWS account is subject to the RCP, tag the bucket, and enable attribute-based access control for the bucket. (1) The tag's value must be a partial ARN of the form \"ACCOUNT:key/ID\" or \"ACCOUNT:key/mrk-ID\"; (2) the specified KMS key, multi-region key primary, or a multi-region key replica must exist in the same region as the bucket; and (3) users' permissions and/or the KMS key policy must allow usage of the key to encrypt S3 objects. Users must specify \"aws:kms\" encryption, and the full KMS key ARN, when creating objects. If the bucket is the destination of a replication rule, the rule's ReplicaKmsKeyID value and the tag's value must reference the same key. Customer-managed KMS keys are recommended. Consider creating keys in a central account, and using multi-region keys. For bucket tag rules, see https://docs.aws.amazon.com/AmazonS3/latest/userguide/tagging.html#tag-key . For ABAC, see https://docs.aws.amazon.com/AmazonS3/latest/userguide/buckets-tagging-enable-abac.html . For S3 KMS key permissions, see https://docs.aws.amazon.com/AmazonS3/latest/userguide/UsingKMSEncryption.html#:~:text=Permissions"
